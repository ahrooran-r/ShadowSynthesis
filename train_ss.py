...
# compute doesn't need to load discriminator
...
if is_training:
    ...
    for epoch in range(1, maxepoch):
        ...
        for id in np.random.permutation(num_train):
            ...
            if input_images_ids[id] is None:
                ...
                iminput, imtarget, maskgt = parpare_image(input_images_path[_id], (neww, newh), da=True,
                                                          stage=['_M', '_C', '_B'])
                ...
                if running_idx % 500 == 0:
                    ...
                    cv2.imwrite("%s/%s_%s.jpg" % (task, running_idx, fileid),
                                np.concatenate((iminput, imoutput, imtarget), axis=1))
    ...
    for val_path in prepare_data([ARGS.data_dir], stage=['shadow_free']):
        ...
        # 100*80
        all_masks = random.sample(
            [os.path.join(mask_dir, x) for x in os.listdir(mask_dir) if os.path.isfile(os.path.join(mask_dir, x))], 3)
